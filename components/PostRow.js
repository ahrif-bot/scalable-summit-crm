    ...
    <div style={styles.card}>
      <div style={styles.cardHeader}>
        ...
        <div style={styles.engSummary}>
          <span style={styles.engBadge('#C8FF00')}>{post.reactions} <span style={{ fontSize: 10 }}>react</span></span>
          <span style={styles.engBadge('#00E5CC')}>{post.comments} <span style={{ fontSize: 10 }}>comm</span></span>
          <span style={styles.engBadge('#FFD93D')}>{post.reposts} <span style={{ fontSize: 10 }}>rep</span></span>
        </div>
      </div>
      ...
      <Field label="Post Summary" value={form.summary} onChange={v => update('summary', v)} multiline />

      <Field label="Tagged People (comma separated)" value={form.tagged} onChange={v => update('tagged', v)} multiline rows={3} />
...
function Field({ label, value, onChange, type = 'text', multiline = false, rows = 4 }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ display: 'block', fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600, marginBottom: 5 }}>
        {label}
      </label>
      {multiline
...
const styles = {
  ...
  cardHeader: {
    ...
    marginBottom: 24, paddingBottom: 18, borderBottom: '1px solid rgba(255,255,255,0.06)',
  },
  ...
  engBadge: (color) => ({
    ...
    fontSize: 14, fontWeight: 700, color, display: 'flex', alignItems: 'baseline', gap: 3,
  }),
  ...
  saveBtn: {
    ...
    fontWeight: 700, fontSize: 14, padding: '10px 28px', borderRadius: 8,
  },
}
